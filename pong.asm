...
DATA SEGMENT PARA 'DATA'
	...
	AI_CONTROLLED DB 0					 ; is right paddle controlled by AI
...
;       Right paddle movement
		CHECK_RIGHT_PADDLE_MOVEMENT:
			...
			JE CONTROL_BY_AI
;			Paddle is controlled by user pressing a key			
			CHECK_FOR_KEYS:
...
;			Paddle is controlled by AI
			CONTROL_BY_AI:
				;check if ball is above the paddle(BALL_Y + BALL_SIZE < PADDLE_RIGHT_Y)
				;if it is, move up
				MOV AX,BALL_Y
				ADD AX,BALL_SIZE
				CMP AX,PADDLE_RIGHT_Y
				...
				;check if ball is below the paddle(BALL_Y > PADDLE_RIGHT_Y + PADDLE_HEIGHT)
				;if it is, move down
				MOV AX,PADDLE_RIGHT_Y
				ADD AX,PADDLE_HEIGHT
				CMP AX,BALL_Y
				...
				;if none of the conditions are true, dont move the paddle(exit paddle movement)
				JMP EXIT_PADDLE_MOVEMENT
